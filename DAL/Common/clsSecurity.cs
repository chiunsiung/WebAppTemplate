...
namespace DAL
{
    ...
    {
        public static bool ValidateSignatureBase64(string signature, string message, string key)
        {
            try
            {
                // Encrypt the message
                string encryptResult = EncryptHMACSHA256_Base64(message, key);
                clsLogger.InfoLog(encryptResult);
        ...
        public static bool ValidateSignature(string signature, string message, string key)
        {
            try
            {
                ...
                clsLogger.InfoLog("Signature Text: " + signature);
                clsLogger.InfoLog("Signature Result: " + encryptResult);
                // Compare the signature
                if ((signature.Equals(encryptResult)))
                {
                    return true;
